...
bool parse_dice(const std::string& spec, int& count, int& sides) {
    ...
    if (d == std::string::npos) {
        return spec.empty() ||
               (std::from_chars(spec.data(), end, count).ptr == end &&
                count >= 1 && count <= max_count);
    }

    const char* count_end = spec.data() + d;
    if (d > 0 &&
        (std::from_chars(spec.data(), count_end, count).ptr != count_end ||
         count < 1 || count > max_count)) {
        return false;
    }

    const char* sides_begin = spec.data() + d + 1;
    return sides_begin != end &&
           std::from_chars(sides_begin, end, sides).ptr == end &&
...
void Roll::handle(const dpp::slashcommand_t& event) const {
    std::string spec;
    if (auto param = event.get_parameter("dice");
        std::holds_alternative<std::string>(param)) {
        spec = std::get<std::string>(param);
    }

    int count = 0, sides = 0;
    if (!parse_dice(spec, count, sides)) {
        event.reply(dpp::message("Invalid dice spec `" + spec + "` — use NdM, e.g. `2d6` (max " +
                                 std::to_string(max_count) + " dice, " +
                                 std::to_string(max_sides) + " sides).")
